...
$(function(){
	$('#connect').click(function(){
		...
		$.get('http://localhost/rocky/api/remotecctv/clientapp/auth', { 'username' : uname, 'password' : passw },
			function(data){
				if (data) {
	...
	$('#updateip').click(function(){
		if (getCookie('devices') && getCookie('user_key')) {
			...
			$.get('http://localhost/rocky/api/remotecctv/key/' + user_key + '/devices/updateip/' + devices,
				function(data){
					if (data) {
						if (data.response_status == '200') {
	...
	if (getCookie('updatefrequency')) {
		$.get('http://localhost/rocky/api/remotecctv/key/' + getCookie('user_key') + '/servertime',
			function(data){
				if (data) {
					if (data.response_status == '200') {
